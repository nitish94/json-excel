document.addEventListener('DOMContentLoaded', () => {
    ...
    function addNestedRow(path, isSingleObject, headers) {
        if (isSingleObject) {
        ...
        current[path[path.length - 1]].push(emptyObj);
        renderTable(currentData);
    }

    function deleteNestedRow(path, nestedIndex, isSingleObject) {
        if (isSingleObject) {
        ...
        showModal('<p>Delete this nested row?</p>', () => {
            ...
            current[path[path.length - 1]].splice(nestedIndex, 1);
            renderTable(currentData);
        });
    }
    ...
    function renderTable(data) {
        ...
        data.forEach((rowObj, rowIndex) => {
            ...
            headers.forEach(key => {
                ...
                td.appendChild(renderValue(rowObj[key], [rowIndex, key], 0, nestedHeaders));
                tr.appendChild(td);
            });
    ...
    function renderValue(value, path, level, nestedHeaders) {
        if (level >= 3) {
            // Max 3 levels, show as string
            const span = document.createElement('span');
        ...
        if (Array.isArray(value)) {
            ...
            nestedDiv.appendChild(createNestedTable(value, path, false, nestedHeaders[path[path.length-1]] || [], level));
            return nestedDiv;
        } else if (typeof value === 'object' && value !== null) {
            // Object treated as one-row nested table
            const nestedDiv = document.createElement('div');
            nestedDiv.className = 'nested-table-container';
            nestedDiv.appendChild(createNestedTable([value], path, true, nestedHeaders[path[path.length-1]] || Object.keys(value), level));
            return nestedDiv;
        } else {
    ...
    function setNestedValue(data, path, val) {
        let current = data;
        for (let i = 0; i < path.length - 1; i++) {
            current = current[path[i]];
        }
    ...
    function createNestedTable(nestedData, path, isSingleObject = false, fixedHeaders = null, level = 0) {
        ...
        addNestedRowBtn.onclick = () => addNestedRow(path, isSingleObject, fixedHeaders || headers);
        ...
        nestedData.forEach((rowObj, nestedIndex) => {
            ...
            delBtn.onclick = () => deleteNestedRow(path, nestedIndex, isSingleObject);
            tdAction.appendChild(delBtn);
            tr.appendChild(tdAction);
            headers.forEach(key => {
                const td = document.createElement('td');
                const value = rowObj[key];
                const newPath = isSingleObject ? [...path, key] : [...path, nestedIndex, key];
                td.appendChild(renderValue(value, newPath, level + 1, {}));
                tr.appendChild(td);
            });
            tbody.appendChild(tr);
        });
